...
import 'package:mydatatools/modules/files/services/scanners/local_file_scanner2.dart';
...
@Deprecated("replaced with local_file_isolate.dart")
  ...
  Future<int> start(
    ...
    int count = await database.computeWithDatabase(
      computation: (db) async {
        ...
        LocalFileScanner2 streamScanner = LocalFileScanner2(
          collection.id,
        ...
        controller.stream.listen(
          (event) async {
            ...
            for (var file in fileList) {
              //TODO this won't work, can't access DB in service in an ISOLATE
              fileFutures.add(
                FileUpsertService.instance.invoke(
                  FileUpsertServiceCommand(file, db),
                ),
              );
            }
                ...
                QueryRow? row =
                    await db
                        .customSelect(
                          ...
                          variables: [Variable.withString(folder.path)],
                        )
                        .getSingleOrNull();
                if (row == null) {
